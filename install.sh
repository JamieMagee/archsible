...
echo 'swap    /dev/disk/by-partlabel/cryptswap    /dev/urandom    swap,cipher=aes-xts-plain64:sha256,size=256' >>$MOUNT/etc/crypttab

# make sure we have a crypttab.initramfs as well
echo 'swap    /dev/disk/by-partlabel/cryptswap    /dev/urandom    swap,cipher=aes-xts-plain64:sha256,size=256' >$MOUNT/etc/crypttab.initramfs
